...
def train(
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(training_dataloader):
            ...
            # Recursive application of the model
            current_input = data.x  # Assuming 'x' is the initial input
            for _ in range(number_of_prediction_steps-1):  # Replace 4 with the number of recursive steps
                model_output = model(current_input, data.edge_index, data.edge_attr)
                current_input = torch.cat((model_output[:,:8], current_input[:,8:]), dim=1)

            final_output = model(current_input, data.edge_index, data.edge_attr)
            # Compare the final output after 4 steps with the target
            y = data.y
            ...
            if i % 10 == 0:
                ...
                print(f"Batch: {i+1}/{len(training_dataloader)}, Avg Loss: {avg_loss:.4f}")
        ...
        # Number of prediction steps in validation
